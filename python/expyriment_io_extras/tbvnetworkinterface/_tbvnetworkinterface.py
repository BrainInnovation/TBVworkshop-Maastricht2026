...
class TbvNetworkInterface(Input, Output):
    ...
    def request_data(self, request, *args):
        ...
        start = get_time()
        self._tcp.clear()
        request = unicode_to_bytes(request) # I changed this
    ...
    def get_current_classifier_output(self):
        ...
        -------
        winning_class : int
            The winning class (1,2, ...,n).
            NOTE: Output is 1-based!
        svm_outputs : list
            The svm outputs for each class pair
        rt : int
            The time it took to get the data.
        ...
        # first 4 bytes = int32
        winning_class = struct.unpack('!i', data[:4])[0]

        # remaining bytes = float32 array
        svm_outputs = [
            struct.unpack('!f', data[i:i + 4])[0]
            for i in range(4, len(data), 4)
        ]

        return (winning_class, svm_outputs, rt)

    def get_current_classifier_probability(self):
        """Get the current classifier output.
        ...
        data, rt = self.request_data(
            "tGetCurrentClassifierProbability")
        # first 4 bytes = int32
        winning_class = struct.unpack('!i', data[:4])[0]
        ...
        return (winning_class, svm_probabilities, rt)
